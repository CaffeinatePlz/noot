...
exports.run = (bot, message, args) => {
    ...
    const x = Math.floor(Math.random() * (nickQuotes.length + taeQuotes.length + abbyQuotes.length + otherQuotes.length));
    if (x <= nickQuotes.length) {
        message.channel.send("\"" + nickQuotes[x] + "\"\n*-THE Nick Hammes*");
    } else if (x > nickQuotes.length && x <= (nickQuotes.length + taeQuotes.length)) {
        message.channel.send("\"" + taeQuotes[(x - nickQuotes.length)] + "\"\n*-Tae!*");
    } else if (x >(nickQuotes.length+taeQuotes.length) && x<= (nickQuotes.length+taeQuotes.length+abbyQuotes.length)){
        message.channel.send("\"" + abbyQuotes[(x - (nickQuotes.length + taeQuotes.length))] + "\"\n*-Abby!*");
    } else {
        message.channel.send("\""+ otherQuotes[(x - (nickQuotes.length + taeQuotes.length+abbyQuotes.length))] + "\"");
    }
};

/**
 * Config for `wholesomequote`
 * Defaults:
 * `enabled`: `true`
 * `guildOnly`: `false`
